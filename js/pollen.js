addLayer("p", {
  ...
  gainMult() {
    ...
    if (hasUpgrade("hi", 21)) mult = mult.times(upgradeEffect('hi', 21))
    if (hasUpgrade("hi", 55)) mult = mult.times("e5e4");
  ...
  gainExp() {
    ...
    if (inChallenge("h", 12)) exp = exp.times(0.5);
    return exp;
  },
  ...
  hotkeys: [
    {
      key: "p",
      description: "P: Reset for pollen",
      onPress() {
  ...
  layerShown() {
    return hasUpgrade("f", 55) || player.p.unlocked;
  },
  upgrades: {
    11: {
      title: "More Flowers",
